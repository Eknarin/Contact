...
public class MemContactDao implements ContactDao{
	...
	/**
	 * Get a contact by searchung title
	 * @param title
	 * @return a contactlist which contains title 
	 */
	public List<Contact> getContactByTitle(String title) {
		List<Contact> contactList = new ArrayList<Contact>();
		for(int i=0; i<contacts.size(); i++){
			if(contacts.get(i).getTitle().contains(title)){
